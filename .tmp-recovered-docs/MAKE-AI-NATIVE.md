# MAKE-AI-NATIVE ΓÇö execute this brief
...
1. `GOAL.md` ΓÇö current focus and non-goals (hard wall)
2. `WHY.md` ΓÇö product model
3. `AGENTS.md` ΓÇö hard rules for coding agents
4. User skill `writing-for-agents` (`~/.agents/skills/writing-for-agents/SKILL.md` + `SKILL-MECHANICS.md`)
5. User skill `ponytail` (full) ΓÇö laziest solution that works
6. User skill `codebase-design` ΓÇö module / interface / depth / seam / locality
7. User skill `skill-design-principles` ΓÇö one home per fact; no no-ops; no bandaids
8. User skill `coding-standards` ΓÇö **principles only** (readability, KISS, DRY, YAGNI). Ignore its TypeScript / React / Next.js examples. This repo is Python 3.11.
9. In-repo `skills/build-job/SKILL.md` ΓÇö Job authoring already exists; do not rewrite it unless a pointer is stale
10. `docs/AGENT_DRILL.md` ΓÇö cold-agent proof for the Job loop
...
**Done when** every acceptance box in ┬º12 is true, and you have **not** implemented Fleet F2, Doctor, vault, UI, schedule, or any `docs/LATER.md` row.

---

## 2. Hard walls (do not cross)

These are product law. A ΓÇ£helpfulΓÇ¥ violation is a fail.
...
## 3. Starting facts (already true ΓÇö do not re-discover blindly)

Verify these. Treat them as the audit seed, not as scripture.

### Product

- Days 1ΓÇô7 complete. Fleet F1 = sibling package `src/coreme_agent/` + CLI `coreme-agent`.
...
| Gap | Evidence |
|-----|----------|
| **AGENTS.md is a cache of GOAL** | Restates Days 1ΓÇô7, Fleet, secrets, phases, repair. High context load every turn. writing-for-agents wants **hard traps + pointers**, not a second product spec. |
...
- **Layered agent context:** `AGENTS.md` = ambient ΓÇ£how we codeΓÇ¥; Skills = invokable ΓÇ£how we ship this kind of changeΓÇ¥; MCP = live data. They are layers, not competitors.
...
- **Plan before large edits.** Wave AΓÇôB are design; wait for human ΓÇ£goΓÇ¥ before rewriting AGENTS.md.
- **Verifiable goals.** Agents iterate against ruff / types / pytest, not vibes.
- **Python 2026 toolchain to evaluate, not blindly adopt:** `uv` + Ruff (lint+format) + `ty` (Astral type checker, ~10ΓÇô60├ù mypy). **Do not replace mypy in this pass** unless you prove `ty` matches current checks and the human agrees. You may add `ty` as a **shadow** command in the verify script.
...
| **map** | `_index.md` path / concept / task rows ΓÇö navigate before grep |
...
**Positive over negation.** Write ΓÇ£open `fail.json` then `log.txt`ΓÇ¥ not a long ΓÇ£do not invent root causeΓÇ¥ sermon. A prohibition earns a line only as a trap you cannot phrase as a target.
...
2. **Grill the human** on the decisions in ┬º6. Batch questions. Recommend defaults.
3. After answers (or if the human says ΓÇ£use your defaultsΓÇ¥), execute **Wave A ΓåÆ E in order**. Stop after any wave if the human says stop.
...
| Nested `AGENTS.md` | Add `src/coreme/AGENTS.md` and `src/coreme_agent/AGENTS.md` only if each has **ΓëÑ3 traps** that do not belong globally. Otherwise one root file + map rows. |
...
| Ruff format | Yes. Match existing line length (measure first; do not invent 88 vs 100 as a debate ΓÇö pick the current median). |
...
| CI | Add a **minimal** GitHub Actions workflow: ruff + mypy + pytest on pull_request/push. Windows latest **or** ubuntu ΓÇö pick one runner first (ubuntu is cheaper; add windows only if path tests need it). Prefer proving path tests already pass on the existing suite. |
| examples/ in gitignore | If examples are tracked: remove `examples/` from `.gitignore`. If they are intentionally local-only: say so in the map and stop. **Recommended:** examples are product ΓÇö they belong in git. |
...
| ponytail-audit | Run it as a **read-only** pass in Wave 0. Apply only deletions that are obviously safe (dead pyc, ignore files). Deeper architecture ΓåÆ list, do not execute. |
...
## 6. Grill the human (batch ΓÇö once)

Ask only if they did not already say ΓÇ£use defaultsΓÇ¥. One message. Recommended default in each option.
...
3. **Ruff aggressiveness.** Safe set in ┬º5.2 (recommended) or also `N` (naming) / `ANN` (annotations)?
...
6. **Scope cap.** All waves AΓÇôE (recommended) or stop after A+B (index + AGENTS + maps) this session?

Then proceed.

---

## 7. Wave 0 ΓÇö Audit (read-only)
...
3. **Hot files.** `git log --oneline -30` ΓÇö where edits actually land.
...
| Area | Now | Cost | Wave |
|------|-----|------|------|
| ΓÇª | ΓÇª | high/med/low | AΓÇôE |

Do not edit product code in Wave 0.

---

## 8. Wave A ΓÇö Faster index (do this first; smallest risk)
...
- [ ] Same patterns mirrored where cheap: `.ignore` or `.repomixignore` (for Repomix / GitIngest packs). Do **not** create five nearly-identical files if one `.cursorignore` + gitignore is enough ΓÇö prefer **one extra ignore file** plus a pointer. If you add `.repomixignore`, it is a thin copy of the cursor ignore.
...
## 9. Wave B ΓÇö AGENTS.md, maps, nested rules
...
1. **Hard traps** (short, checkable) ΓÇö examples that belong:
   ...
   - Verify command (pointer to script or the four commands ΓÇö not both)
2. **Discovery protocol** ΓÇö numbered: GOAL ΓåÆ (day plan if named) ΓåÆ WHY ΓåÆ this file ΓåÆ `_index.md` ΓåÆ owning module ΓåÆ skill
3. **Pointers with leading words and branches** ΓÇö one trigger per branch. Example:
   - **Job** author / change / prove / ship ΓåÆ `skills/build-job/SKILL.md`
   - **Repair** failed Run / brief / auto-repair ΓåÆ `skills/build-job/repair.md`
   - **Phases** / seed-from-fail ΓåÆ `skills/build-job/phases.md`
   - **Kernel implement** ΓåÆ `skills/devex/implement-kernel.md` (after Wave D)
   - **Debug kernel / test red** ΓåÆ `skills/devex/debug-kernel.md`
   - **Maps** ΓåÆ `_index.md`
4. **Style one-liner** that points at the linter: ΓÇ£Python 3.11, stdlib first, Windows paths; `ruff` + `mypy` are the style guide.ΓÇ¥

**Delete** restated Day 1ΓÇô7 narrative, Fleet essays, and operator-report dumps from AGENTS.md. Those live in GOAL / day plans / skills.

**Budget:** aim **Γëñ 80 lines**. If you need more, you failed to disclose.

### 9.2 Add root `_index.md`

Keep ~40ΓÇô60 lines. Sections:

- **Status** ΓÇö one line: Days 1ΓÇô7 done; Fleet F1 done; F2 next when GOAL says; this repo is a Job kernel + sibling agent package
- **Path map** ΓÇö `src/coreme/`, `src/coreme_agent/`, `skills/build-job/`, `examples/`, `tests/`, `docs/`
- **Concept map** (product language ΓåÆ owning module ΓåÆ start file). Required rows at least:
...
- **Task map** ΓÇö class of work ΓåÆ read first:
...
Only if the grill default in ┬º5.2 says so **and** you found real local traps (e.g. agent package: SQLite queue, never import from `coreme` internals the wrong way ΓÇö **verify** the actual import rule before writing it).
...
**Done when:** a stranger can answer ΓÇ£where does resolve live?ΓÇ¥ from `_index.md` without grep; AGENTS.md no longer restates GOAL; `docs/README.md` points at the map.

---

## 10. Wave C ΓÇö Lint, format, types, verify, CI
...
- Enable lint: existing + `I`, `UP`, `SIM`, `RUF`, `PTH` (pathlib ΓÇö this codebase is Windows-path sensitive). Keep `B`.
...
- If a rule fights a hard product choice (e.g. a deliberate `print(..., flush=True)`), configure that rule ΓÇö do not scatter `# noqa` without a comment naming why.
...
## 11. Wave D ΓÇö Skills (few, sharp)
...
2. Open `_index.md` ΓåÆ owning module ΓåÆ local AGENTS if any.
...
Branches: test red, CLI unexpected exit, failed Run, flaky path, ΓÇ£it works in chat but proof failsΓÇ¥.
...
2. If a Run exists: `fail.json` ΓåÆ `coreme brief` ΓåÆ `events.jsonl` ΓåÆ `log.txt`. Do not invent cause.
3. 3ΓÇô5 falsifiable hypotheses. Change one variable.
...
- ΓÇ£75 skillsΓÇ¥ pack
- Duplicate `build-job` as ΓÇ£build-kernelΓÇ¥
...
## 12. Wave E ΓÇö Faster implement + debug (docs and loops, not platform)
...
```text
map ΓåÆ smallest test or example ΓåÆ code ΓåÆ scripts/verify.ps1 ΓåÆ update map if structure moved
```
...
```text
tight red command ΓåÆ evidence (fail.json / brief / events) ΓåÆ hypotheses ΓåÆ one-variable probe ΓåÆ fix at shared seam ΓåÆ regression ΓåÆ verify
```
...
One short ΓÇ£DevelopΓÇ¥ subsection: install `.[dev]`, run `scripts/verify.ps1`. Do not turn README into AGENTS.md.

### 12.5 Ideas you may implement if still cheap after AΓÇôD

Only if each is < ~20 minutes and ponytail-clean:

| Idea | Do if |
|------|--------|
| `ruff` pre-commit config | You already wanted CI; pre-commit is optional duplicate ΓÇö **skip** if CI + verify script exist |
...
- [ ] Root `AGENTS.md` is traps + pointers, not a GOAL clone; roughly Γëñ 80 lines
- [ ] Root `_index.md` has path, concept, and task maps; concept rows use product words (Job, Run, Release, ΓÇª)
- [ ] `docs/README.md` updated; no second copy of the maps
- [ ] Ruff expanded + format check; mypy still gate; verify script green
- [ ] CI runs the same checks
- [ ] Γëñ 3 new devex skills; build-job not rewritten
...
2. **Kernel coding standard as ruff only** ΓÇö delete remaining style prose.
3. **Hot-spot deepening** (`cli.py`, `runner.py`, repair trio) via `improve-codebase-architecture` ΓÇö design-it-twice, then one deepening. Not a rewrite.
...
6. **uv lock** for reproducible agent installs (`uv sync --extra dev`) ΓÇö only with human OK.
7. **Windows + ubuntu CI** once ubuntu is green.
8. **Repomix kernel profile** for pasting into web LLMs.
9. **`coreme` help text as source of truth** ΓÇö AGENTS never lists flags.
...
17. Hooks that re-run verify on stop ΓÇö only after verify is boringly green; cap iterations.
18. Semantic-search hints in `_index.md` (synonyms: ΓÇ£freezeΓÇ¥ ΓåÆ ship, ΓÇ£packetΓÇ¥ ΓåÆ Job).
19. Measure index time before/after ignores (Cursor / tool logs) and record the number in the report.
20. Cold **kernel** drill (sibling of AGENT_DRILL): ΓÇ£add a flag that is rejected if unknownΓÇ¥ without reading chat history.

### Reject (name them so future you does not build them)

- A second orchestrator for agents
- Generated always-on ΓÇ£repo digestΓÇ¥ committed to git
- Copying 75 public agent skills into this repo
- Putting hub HTTP in the kernel to ΓÇ£help the AIΓÇ¥
...
Index: ΓÇª
AGENTS / maps: ΓÇª (line counts before ΓåÆ after)
Lint / types / CI: ΓÇª
Skills added: ΓÇª
Verify: <paste>
Skipped (ponytail): ΓÇª
Next (pick one): ΓÇª
