...
The **Job process** never calls an LLM. Default `coreme run` does not need the chat transcript or agent API keys. **Repair** uses the failed Run as the brief and patches **source** only. Day 7 adds manual repair CLI and **optional automatic** host Codex deploy after a failed Run (`--auto-repair` / env) ΓÇö one command for ops, like a thin nga recovery path, not a Doctor daemon or auto-ship.

**Agent-time steps** (clarify ΓåÆ prove ΓåÆ ship) live in the skill and chat ΓÇö not a delivery-plan product.
**Runtime phases** (login ΓåÆ download ΓåÆ report) live inside one Job as sequential steps wired by **artifacts**; Day 5 formalizes Job-owned `only` / `skip` for debug (see `examples/phased-demo`).
Neither is roi-hΓÇÖs phase graph nor ngaΓÇÖs delivery OS. See [docs/LESSONS-phases.md](docs/LESSONS-phases.md).
...
Vaults, browser engines, dashboards, SQLite, schedulers, agent RPC, full Doctor (draft/activate/reviews), heal-until-green daemons, fleet, and platform UX are deferred ΓÇö see [docs/LATER.md](docs/LATER.md). Day 7 is thin repair: brief + optional auto Codex on fail under policy.

## North star

Grow one proven layer at a time: **Job ΓåÆ prove ΓåÆ parameterize ΓåÆ freeze ΓåÆ re-run** (then secrets, phases, operator report + run events, then agent-time repair).
