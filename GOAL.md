# GOAL.md — coreme north star and plan tracker
...
- **Default `coreme run`** (auto-repair off): no Codex — works offline with no agent API keys.
- **Day 7 repair:** (1) manual `coreme brief` / `coreme repair [--exec]`; (2) **automatic** after a failed Run when `--auto-repair` or `COREME_AUTO_REPAIR=1` — one command deploys host `codex exec` with the fail brief (nga-shaped). Max one Codex spawn per run; no auto-ship; no edit of `releases/`.
- Skills are Markdown guidance for agents; auto-repair is a thin post-fail coordinator, not a Doctor daemon.

**Two kinds of “phases” (do not mix):**

| Kind | Where | Examples |
|------|--------|----------|
| **Agent-time** | Skill + chat | Clarify → explore → contract → code → test → run → ship |
| **Runtime** | Inside one Job entry | login → download → process → report (`only` / `skip` on Day 5) |
...
| Repair brief + opt-in auto Codex on failed run | Day 7 thin CLI + skill — not full Doctor product |
...
Milestones **0–3 are the real product.** Day 4–5 finish the everyday author/debug loop. Packaging beyond that is [docs/LATER.md](docs/LATER.md).

| # | Name | What you can say | Spec | Status |
|---|------|------------------|------|--------|
| **0** | Toy job by hand | “I ran one automation with a run folder” | — | ✅ done (via Day 1) |
| **1** | Kernel CLI | “`coreme init/test/run` works without AI” | Day 1 | ✅ **done** |
| **2** | Inputs + harden | “Same Job, different data; fail paths honest” | [docs/days/DAY2.md](docs/days/DAY2.md) | ✅ **done** |
| **3** | Ship | “Dev source freezes to an immutable release” | [docs/days/DAY3.md](docs/days/DAY3.md) | ✅ **done** |
| **4** | Secrets (thin) | “Secrets not in repo; env only” | [docs/days/DAY4.md](docs/days/DAY4.md) | ✅ **done** |
| **5** | Job-owned phases | “Run or skip Job steps for debug with normal inputs” | [docs/days/DAY5.md](docs/days/DAY5.md) | ✅ **done** |
| **6** | Operator report + run events | “Pretty TTY; plain `log.txt`; structured `events.jsonl`; agent skill” | [docs/days/DAY6.md](docs/days/DAY6.md) | ✅ **done** |
| **7** | Repair loop (+ auto) | “Failed Run → brief; prod `--auto-repair` deploys Codex without a second command” | [docs/days/DAY7.md](docs/days/DAY7.md) | ✅ **done** |

**Post–Day 5 polish (in tree, not a formal day):** bare process name → latest release; live unbuffered stream to terminal + `log.txt`; operator UX + browser skill docs; `examples/browser-stub`; ops handoff template.
...
|-----|------|------|--------|
| **1** | Job / Run / Runner CLI; hello example; WHY + AGENTS + build-job skill | Implemented in repo | ✅ **passed** |
| **2** | Declared inputs; fail/timeout tests; entry containment; `examples/greet` | [docs/days/DAY2.md](docs/days/DAY2.md) | ✅ **done** |
| **3** | `coreme ship` + content hash + verify on run | [docs/days/DAY3.md](docs/days/DAY3.md) | ✅ **done** |
| **4** | Thin secrets: declare names; env at run; structured evidence stores names only | [docs/days/DAY4.md](docs/days/DAY4.md) | ✅ **done** |
| **5** | Job-owned phases: sequential steps; `only` / `skip`; seed file | [docs/days/DAY5.md](docs/days/DAY5.md) | ✅ **done** |
| **6** | Operator report skills; Rich CLI; plain `log.txt`; `events.jsonl` | [docs/days/DAY6.md](docs/days/DAY6.md) | ✅ **done** |
| **7** | Repair: skill + brief/repair CLI + auto Codex deploy on fail (policy) | [docs/days/DAY7.md](docs/days/DAY7.md) | ✅ **done** |
| **fleet** | Multi-machine agent + thin hub (LATER #7 schedule slice + #9 fleet) | [docs/days/FLEET.md](docs/days/FLEET.md) | ✅ **F0–F3** in code; next **F4** per docs/PLAN.md W3 |
| **later** | Other items from [docs/LATER.md](docs/LATER.md) with a new plan under `docs/days/` | — | ⬜ |
...
- [x] Missing/empty env secret → exit 2 before Run folder
...
- [x] Host-setup polish — superseded by secrets ceremony in skill + [skills/build-job/OPS-TEMPLATE.md](skills/build-job/OPS-TEMPLATE.md)
...
**Fleet** is active via [docs/days/FLEET.md](docs/days/FLEET.md): F1–F3 are done in code (hub claim, lease reclaim, release pull, evidence upload); F4 (metrics) is next per docs/PLAN.md W3. Prefer that ladder over inventing Doctor/platform. Other LATER rows still need their own day plan.

Post–Day 5 polish remains in tree: process-name resolve, live stream, operator UX / browser skill, `browser-stub`.
...
- Secrets vault / Credential Manager / keyring / Coreme secret-store file / `.env` loader / encryption at rest — **beyond Day 4 thin names** (see [docs/days/DAY4.md](docs/days/DAY4.md))
...
## Definition of “reproducible automation”
...
4. Declared inputs — not hidden agent memory — parameterize the run.
...
7. Ops can re-run by process name against the latest release from the workspace root (post–Day 5 polish).
...
```text
1. Read GOAL.md (this file) — know the focus and non-goals
...
6. Stop when the checklist is done; update this file’s status boxes
7. Do not implement docs/LATER.md items without a new docs/days/ plan
```

Human UX:

> Open agent → “Build an automation that …”  
...
| [docs/days/DAY3.md](docs/days/DAY3.md) | Day 3 plan (ship / hash) — sealed |
| [docs/days/DAY4.md](docs/days/DAY4.md) | Day 4 plan (thin secrets) — sealed |
| [docs/days/DAY5.md](docs/days/DAY5.md) | Day 5 plan (Job-owned phases) — sealed |
| [docs/days/DAY7.md](docs/days/DAY7.md) | Day 7 plan (agent-time repair / Codex) — **done** |
| [docs/days/FLEET.md](docs/days/FLEET.md) | Fleet ladder F0–F7 (agent + hub; **F0–F3 done in code, F4 next**) |
...
| [skills/build-job/operator-report.md](skills/build-job/operator-report.md) | Operator report / UX (steps, ÖZET, idle) |
...
| 2026-08-07 | Quality pass: `paths`/`proof`/`util` split (no ship↔runner cycle; proof timeout; JobPathError); `tests/helpers.py`; docs note product-vs-tests. Still no Day 4. |
...
| 2026-08-07 | Plan reset: old milestones 5–10 moved to docs/LATER.md. Day 4 secrets treated as done in tree. Active calendar = Day 5 Job-owned phases (`only`/`skip`, seed file). build-job skill gains clarify/explore + runtime phase pattern. Day 5 not implemented. |
...
| 2026-08-07 | Post–Day 5 ops polish: bare-name → latest release (`resolve.py`); live unbuffered stream; operator-ux / browser / OPS-TEMPLATE skills; `examples/browser-stub`. Kernel LOC ~1063. Day 4 host-setup checkbox closed (superseded by skill ceremony). |
...
| 2026-08-07 | Skill-only: batch grill in build-job clarify ([skills/build-job/clarify.md](skills/build-job/clarify.md)) — unknowns + recommended defaults + assumptions; no kernel, no decisions tree (nga idea, not delivery OS). |
| 2026-08-07 | Post–Day 6 polish: kernel `fail.json` on failed Runs (kind/message/step brief + `coreme events` FAIL header + plain footer `fail_path`); operator-facing **contract spine** (grill → contract → steps → ÖZET) in build-job skill. Not full repair loop / Doctor. Days 1–6 remain done; focus still ship real Jobs or one LATER item. |
...
| 2026-08-08 | **Post–Day 7 helpers (not a formal day):** (A) `coreme seed-from-fail` stages Run artifacts + prints/execs seeded re-run; (B) Job-local `JOB.md` + template + init stub; (G) Windows `make-shortcut.ps1` + OPS section. No resume protocol / Control Room / scheduler product. |
...
| 2026-08-21 | Docs restored from .tmp-recovered-docs into root+docs layout; adopted docs/PLAN.md (W0-W4). Fleet F2/F3 code exists; ladder checkboxes reconciled. |
...
1. Check off the day’s boxes above.
2. Set **Current focus** at the top to the next day (or “done; ship Jobs / LATER”).
