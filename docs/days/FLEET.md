# Fleet plan — agent + hub (multi-machine coreme)

**Status:** 🔄 **active — F1–F3 done in code; next F4 per [docs/PLAN.md](../PLAN.md) W3** ([GOAL.md](../../GOAL.md) **Current focus** governs pacing).  
...
**Product one-liner:**  
**coreme** freezes and runs one Job. **coreme agent** (Windows) heartbeats, claims work, pulls a hashed release, runs `coreme`, ships evidence. **coreme hub** is a thin queue + machine registry + release catalog over Postgres — not a second Studio.

**Grill settled (2026-08-08):**

| Decision | Choice |
|----------|--------|
| Plan shape | Full ladder **F0–F7** in this one file; implement only the stage GOAL names |
| Tenancy | **Single operator / org** — customers/sites are **tags**, not tenants |
...
- One Assignment → one local `coreme run` → one Attempt. Fan-out = many Assignments (same release, different inputs).
- Lease default **15 minutes**, renew while running; expired lease → reclaimable.
...
| Object | Meaning |
|--------|---------|
| **Job / Release** | Unchanged. `coreme ship` → content hash; agent refuses dirty/mismatched bits |
| **Machine** | Windows PC: `id`, tags, last heartbeat, agent version, status |
| **Assignment** | “Run this release (hash) with these inputs on a machine matching tags” |
| **Attempt** | One claim → local Run → complete/fail/timeout/lease-lost |
...
Claim matches **required tags ⊆ machine tags**. No per-customer database isolation product in this plan.
...
Deep debug still prefers the machine’s Run folder when reachable; hub is ops index + fail bodies.
...
┌──────────────────────┐          ┌─────────────────────────────┐
│ coreme agent         │ HTTPS    │ coreme hub                  │
│  (service / process) │ outbound │  Postgres                   │
│  tags, machine token │◄────────►│  machines / assignments /   │
│                      │ claim    │  attempts / release catalog │
│ coreme run ──────────┤ renew    │                             │
│ pull release@hash    │ complete │ admin API (create assign,   │
│ local runs/          │ evidence │ list machines, schedule)    │
└──────────────────────┘          └──────────────┬──────────────┘
                                                 │ /metrics + logs
                                                 ▼
...
**Role-split across hosts** (download on A, process on B): two Jobs + artifact store / shared path — **not** a kernel DAG (pattern B, later if needed).  
...
| Qubitory Orchestrator | “Work unit + machine match” | Canvas, multi-tenant, Human Action, production listeners dual-authority, built-in Studio UI |
| coreme Day 3–7 | Hash verify, Run evidence, fail brief, auto-repair on machine | Putting fleet inside kernel |
| LATER #7 | Schedule = enqueue Assignments (hub or OS) | In-kernel cron on agent |
| LATER #10 | — | Custom dashboard product; use Grafana |
...
```text
src/coreme/              # kernel — unchanged product surface (no hub HTTP)
...
POST /v1/assignments/{id}/renew
  → extends lease if still claimed by this machine
...
POST /v1/assignments/{id}/evidence
  multipart or presigned upload — full tree when status=fail (F3)
...
POST /v1/schedules         # F5: cron → create assignments
...
### F0 — Design sealed (this file)

**You can say:** “Fleet objects, wall, non-goals, and ladder are written; no fleet code required yet.”
...
### F1 — Local agent + local queue (single machine)

**You can say:** “One Windows agent drains a local queue, runs `coreme` on a pinned release path, writes Attempt outcome next to the queue — no multi-machine network yet.”
...
| [x] Create Assignment offline (CLI) → agent runs → success/fail recorded (SQLite Attempts) |
...
| Enqueue | `coreme-agent enqueue --release <path> [--input k=v] [--id …]` |
...
### F2 — Hub API + multi-machine claim + heartbeat

**You can say:** “Two Windows PCs heartbeat to one hub; only one claims a given Assignment; lease expires and work is reclaimed.”
...
| Done means |
|------------|
| [x] Hub migrates schema and serves claim loop |
| [x] Two agents (or two machine_ids in test) do not double-run same Assignment under lease |
| [x] Expired lease → second machine can claim |
| [x] e2e or integration test with testcontainers/local Postgres |

Proven by `tests/test_hub.py` (2026-08-21): claim fencing, lease reclaim, agent-vs-hub e2e.
...
### F3 — Release pull + evidence upload

**You can say:** “Agent pulls release bits from hub catalog URL, verifies content hash, runs, always posts summary; on fail uploads full Run tree.”

| Ship | Detail |
|------|--------|
| Release catalog | Register release + `blob_url` + `content_hash` |
| Agent pull | Download to cache dir → verify hash → `coreme run` that tree (or unpack layout matching release folder) |
| Evidence | Summary on every complete; full tree multipart/presigned on fail |
| Cache | Reuse cached hash; no re-download if match |

| Done means |
|------------|
| [x] Dirty or wrong hash → refuse run, fail Attempt with clear error |
| [x] Success Attempt has summary on hub; fail Attempt has full tree retrievable |
| [x] Secret values never in Assignment JSON or evidence index |

Proven by `tests/test_hub.py` (2026-08-21): wrong-hash refusal, fail-tree upload, secret-name-only index.

**Non-goals F3:** CDN product, signing, multi-region storage product.

---

### F4 — Metrics + log ship + Grafana starter

**You can say:** “I open Grafana and see machine up, claim latency, run duration, fail rate; logs land in Loki.”
...
**You can say:** “A cron expression on the hub creates Assignments on a timer; agents still only pull and run.”
...
**You can say:** “I can drain a machine, pin agent version expectations, and trust reclaim under network blips.”

| Ship | Detail |
|------|--------|
| Drain | Machine flag: no new claims; finish current |
| Lease renew | Reliable renew during long Jobs |
| Stale machines | Heartbeat timeout → show offline in list/metrics |
| Version pin (light) | Optional min agent version on Assignment or hub config |
| Retries policy | Document: reclaim ≠ automatic N retries storm; optional max_attempts on Assignment |
...
### F7 — Polish + skill/docs freeze

**You can say:** “Cold agent can install agent, register machine, enqueue fan-out, and read fail evidence from hub without inventing a platform.”
...
| [ ] Ladder F1–F6 green on a two-machine lab (or documented CI substitute) |
...
| LATER | Covered by |
|-------|------------|
| #7 Schedule + notify | **F5** schedule enqueue; notify = metrics/alerts later (optional webhook thin slice only if needed — not required for F5 done) |
| #8 Multi-job workspace | Unchanged local layout; fleet adds many Runs across machines, not a monorepo feature |
| #9 Fleet | **F1–F6** |
| #10 Platform | **Not** this plan — Grafana + optional thin JSON lists only |
...
- TLS termination in front of hub (Caddy/nginx/cloud LB) — hub may listen plain on localhost behind proxy.  
...
1. Confirm Days 1–7 remain the local Job product (no kernel rewrite).  
2. Set GOAL **Current focus** to **Fleet F\<n\>** (start at **F1** after F0 checklist admin bits).  
3. Implement only that stage’s **Done means**.  
4. Do not implement chatbots, multi-tenant, or Canvas “while here.”
...
- Windows service wrapper (NSSM vs native vs Task Scheduler “at logon”)  
